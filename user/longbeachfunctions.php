            ...
            <ul>
                ...
                for ($cb = 1; $cb <= $boxDisplay; $cb++) {
                    if (($cb >= $currentMonthFirstDay + 1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)) {
                        ...
                        $result = $db->query("SELECT title FROM longbeachbookings WHERE date = '" . $currentDate . "' AND status = 1");
...
function getEvents($date = '') {
    ...
    $result = $db->query("SELECT title FROM longbeachbookings WHERE date = '" . $date . "' AND status = 1");
...
function addEvent($date, $title, $user_id, $fname, $address, $contact, $email, $guests, $hour, $minutes, $duration, $confirmation) {
    ...
    $insert = $db->query("INSERT INTO longbeachbookings (title,date,created,modified,user_id,fname,address,contact,email,guests,hour,minutes,duration,confirmation) VALUES ('" . $title . "','" . $date . "','" . $currentDate . "','" . $currentDate . "','" . $user_id . "','" . $fname . "','" . $address . "','" . $contact . "','" . $email . "','" . $guests . "','" . $hour . "','" . $minutes . "','" . $duration . "','" . $confirmation . "')");
    if ($insert) {
        echo 'ok';
    } else {
        echo 'err';
