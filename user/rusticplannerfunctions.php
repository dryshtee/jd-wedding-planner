   ...
   <tr>
            ...
            <ul>
                ...
                for ($cb = 1; $cb <= $boxDisplay; $cb++) {
                    if (($cb >= $currentMonthFirstDay + 1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)) {
                        ...
                        $result = $db->query("SELECT title FROM kvsbookings WHERE date = '" . $currentDate . "' AND status = 1");
    ...
    <script type="text/javascript">
        function getCalendar(target_div, year, month) {
            $.ajax({
                type: 'POST',
                url: 'rusticplannerfunctions.php',
...
function getEvents($date = '') {
    ...
    $result = $db->query("SELECT title FROM kvsbookings WHERE date = '" . $date . "' AND status = 1");
