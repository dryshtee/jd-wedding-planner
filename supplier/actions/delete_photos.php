...
?>
<?php
$res = mysql_query("SELECT * FROM photosbookings WHERE id='$id'");
while ($row = mysql_fetch_array($res)) {
    ...
    if (isset($_POST['delete'])) {
        ...
        $query3 = mysql_query("DELETE FROM photosbookings WHERE  id='$id'", $conn);
        if ($query3) {
            echo "<script>alert('Record deleted Successfully')</script>";
            echo "<meta http-equiv='refresh' content='0;url=../supplier/sphotography.php'>";
        }
    }
    ?>
    <body>
